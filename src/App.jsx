...
export default function App() {
  ...
  // and returning to page 2 without changing active cadets keeps everything.
  const [session, setSession] = useState(null); // { scores, obsScores, weights, selectedVariant }

  const handleContinue = (data) => {
    setSession((prev) => {
      // If the active-cadet set is unchanged, keep existing session data
      if (
        prev &&
        setup &&
      ...
      // but preserve any scores for cadets that are still active.
    ...
    <div dir="rtl">
      <header className="bg-ink border-b-[3px] border-gold">
        <div className="max-w-[1100px] mx-auto py-[18px] px-6 flex items-baseline gap-3">
          <span className="font-black text-[22px] text-cream tracking-tight">גיבוש</span>
          <span className="text-[13px] text-brand-sub font-medium">מערכת הערכת מלש״בים</span>
        </div>
      </header>
