...
// Builds a fixed histogram's shape (groups 1-7) and fills it with student
...
// groups 1/7 the extremes, based on each student's percentile rank.
export function computeLiveGroups(activeCount, rankMap) {
  const groups = {};
  Object.entries(rankMap).forEach(([student, rank]) => {
    if (activeCount === 1) {
      groups[student] = 4;
      return;
    }
...
export function buildLiveHistogramGrid(activeStudentNums, groups) {
  const byGroup = { 1: [], 2: [], 3: [], 4: [], 5: [], 6: [], 7: [] };
  activeStudentNums.forEach((c) => {
    const g = groups[c];
